...
class User {
    ...
    public getField=(fieldName:StringKeys<IUserData>):string=>{
        switch (fieldName){
          ...
          default: throw new Error(`Invalid fiel name: ${fieldName}`);
        }
    }
    ...
    public static _validateName=(value:string):ErrorType[]=>{
        ...
        if (value!==null && !pattern.test(value) ) errors.push({errorNumber:1,message:'Имя должно содержать только буквы'});
        return errors;
    }
    ...
    public static _validatePhone=(value:string):ErrorType[]=>{
        ...
        const errors:ErrorType[]=[];
        if (value!==null && !pattern.test(value) ) errors.push({errorNumber:1,message:'Телефон должен содержать только цифры'});
        return errors;
    }
    ...
    public static _validateEmail=(value:string):ErrorType[]=>{
        ...
        const errors:ErrorType[]=[];
        if (value!==null && !pattern.test(value) ) errors.push({errorNumber:1,message:'Email должен содержать @'});
        return errors;
    }

    public static validate(fieldName:StringKeys<IUserData>,value:string):ErrorType[]{
        switch (fieldName){
          case 'name':  return this._validateName(value);
          case 'phone': return this._validatePhone(value);
          case 'email': return this._validateEmail(value);
          default: throw new Error(`Invalid fiel name: ${fieldName}`);
        }
        return [];
    }
